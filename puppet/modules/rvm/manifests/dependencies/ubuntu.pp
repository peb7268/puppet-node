class rvm::dependencies::ubuntu {
  ...
  if ! defined(Package['libxml2-dev'])          { package { 'libxml2-dev':          ensure => present, require => Exec["update deps"] } }
  if ! defined(Package['libxslt1-dev'])         { package { 'libxslt1-dev':         ensure => present, require => [Exec["update deps"]], alias => 'libxslt-dev' } }
